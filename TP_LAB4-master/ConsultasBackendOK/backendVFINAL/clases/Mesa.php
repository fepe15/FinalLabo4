...
public static function NoSeUso()
    ...
    {
        $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso();
        $consulta = $objetoAccesoDato->RetornarConsulta("SELECT SUM(prod.precio) as total from productos as prod where nombre in (SELECT pd.producto FROM pedidodetalle as pd WHERE pd.mesaPedido = $this->idMesa and pd.estado='listo para servir')");
        $consulta->execute();
